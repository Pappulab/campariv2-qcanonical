...
!       disulfide linkage
 ...
 & This is most likely an omission bug.'
    ...
    freesolv_limits(i,:,:) = 999.0 !set to a dummy number that won't come up in param file
!
...
!   supported of course), unsupported ones require patch
 ...
 &      (resname.eq.'R5P').OR.(resname.eq.'UNK')) then
      ...
      freesolv(i,1:3) = FOS_S1P(1:3)
    else if (resname.eq.'S2P') then
      freesolv(i,1:3) = FOS_S2P(1:3)
    else if (resname.eq.'SXP') then
      freesolv_limits(i,1:3,1) = FOS_S2P(1:3)
      freesolv_limits(i,1:3,2) = FOS_S1P(1:3)
      
    else if (resname.eq.'THR') then
      freesolv(i,1:3) = FOS_THR(1:3)
    else if (resname.eq.'T1P') then
      freesolv(i,1:3) = FOS_T1P(1:3)
    else if (resname.eq.'T2P') then
      freesolv(i,1:3) = FOS_T2P(1:3)
    else if (resname.eq.'TXP') then
      freesolv_limits(i,1:3,1) = FOS_T2P(1:3)
      freesolv_limits(i,1:3,2) = FOS_T1P(1:3)
    ...
    else if (resname.eq.'TYX') then
      freesolv_limits(i,1:3,1) = FOS_TYO(1:3)
      freesolv_limits(i,1:3,2) = FOS_TYR(1:3)
    else if (resname.eq.'Y1P') then
      freesolv(i,1:3) = FOS_Y1P(1:3)
    else if (resname.eq.'Y2P') then
      freesolv(i,1:3) = FOS_Y2P(1:3)
    else if (resname.eq.'YXP') then
      freesolv_limits(i,1:3,1) = FOS_Y2P(1:3)
      freesolv_limits(i,1:3,2) = FOS_Y1P(1:3)
    ...
    else if (resname.eq.'ASX') then
      freesolv_limits(i,1:3,1) = FOS_ASP(1:3)
      freesolv_limits(i,1:3,2) = FOS_ASH(1:3)
      ...
      freesolv_limits(i,1:3,1) = FOS_GLU(1:3)
      freesolv_limits(i,1:3,2) = FOS_GLH(1:3)
      ...
      freesolv_limits(i,1:3,1) = FOS_LYS(1:3)
      freesolv_limits(i,1:3,2) = FOS_LYD(1:3)
    ...
    else if (resname.eq.'HDX') then
      freesolv_limits(i,1:3,1) = FOS_HIP(1:3)
      freesolv_limits(i,1:3,2) = FOS_HID(1:3)
    else if (resname.eq.'HEX') then
      freesolv_limits(i,1:3,1) = FOS_HIP(1:3)
      freesolv_limits(i,1:3,2) = FOS_HIE(1:3)
      
    else if (resname.eq.'HIX') then
      freesolv_limits(i,1:3,1) = FOS_HIP(1:3)
      freesolv_limits(i,1:3,2) = FOS_HIE(1:3)
      ...
      freesolv_limits(i,1:3,1) = FOS_NA(1:3)
      freesolv_limits(i,1:3,2) = 0.0
    else if (resname.eq.'CLX') then
      freesolv_limits(i,1:3,1) = FOS_CL(1:3)
      freesolv_limits(i,1:3,2) = 0.0
 ...
 &e ',resname,' at the moment. Check back later(1).'
...
!#tyler pka:
  do i=1,nseq
    if (freesolv_limits(i,1,2).eq.999.0) then
      freesolv_limits(i,:,1) = freesolv(i,:)
      freesolv_limits(i,:,2) = freesolv(i,:)
    end if
  end do
!
end
...
!---------------------------------------------------------------------
!
subroutine init_svte(isvmode)! This bit of code needs a lot more comments
  use atoms
  use iounit
  use sequen
  use polypep
...
!  use energies
  ...
  if (isvmode.eq.0) then ! Martin : so this is called 
    do i=1,n
      svte(i) = 0.0
      svbu(i) = 0.0
    end do
  else if (isvmode.eq.1) then
    do i=1,n! martin : resets all the svte values while saving them into svbu
      svbu(i) = svte(i)! martin : looks that this whole svbu thing is a cheap savings of variable, makes it hard to understand
      !as it seems that svte is both the previous and the difference in sav
      svte(i) = 0.0
    end do
    
  else if (isvmode.eq.2) then 
    do rs=1,nseq
      do i=1,at(rs)%nbb+at(rs)%nsc !martin : this is to get the indexes rights, as it is in the innerlooop
        ...
        if (svte(ii).ne.0.0) then
          rs_vec(rs) = 1
          if (which_fosg(ii).gt.100) rs_vec2(rs+1) = 1! martin : that is because in the case where the solvation groups is not part of the residue 
        end if
      end do
    end do
    
  else if (isvmode.eq.3) then ! Martin : This list seems is the initialization is IMPSOLV is used
      do i=1,n
        svte(i) = atstv(i) !  It is for initialization
      end do
  else if (isvmode.eq.4) then

    do i=1,n
        
      atsav(i) = atsav(i) - svte(i) ! Martin : this is the cancelling move 
    end do
    ...
    do i=1,n
      ...
      atsav(i) = svte(i)! Martin : sets up the sav as the original infered from the topology
    end do
...
!
 47   format(i10,1000000(1x,g14.7))
!
...
!
 ...
 48   format(f8.3,1x,100000(g13.6,1x,g13.6,1x))
 49   format('#',8x,100000(10x,i8,10x))
!
...
!------------------------------------------------------------------------------------------
!
subroutine en_totsav(evec)
!
  use energies
!
  implicit none
!
  RTYPE tsv,evec(MAXENERGYTERMS),dummy
  logical sayno
!
  sayno = .false.
  tsv = 0.0
...
!
  evec(13) = par_IMPSOLV(10)*(tsv-sysvol(2)) + &! Martin : I seemed to have commented that for some reason. Restoring now.
 &           par_IMPSOLV(11)*sexv
!!
end
!
!----------------------------------------------------------------------------------------------------
! 
subroutine en_totsav2(evec,mode)
!
  use energies
!
  implicit none
!
  integer mode
  RTYPE tsv,tsv2,evec(MAXENERGYTERMS)
  logical sayyes
!
  sayyes = .true.
...
!
  if (mode.eq.1) then
!   here we have to cheat a little bit: mode 2 means the fxn was called with the prior configuration
!   assuming the posterior energies have been computed already.
!   however, those aren't available until now, so this term is not computed.
!   hence, we compute the negative difference (see minus sign below) and simply write it to the prior(!) energy vector
    if (par_IMPSOLV(10).ne.0.0) call get_molsav(tsv,tsv2,sayyes)
  end if
!
  if (mode.eq.1) then
    if (par_IMPSOLV(10).ne.0.0) then
      evec(13) = evec(13) - par_IMPSOLV(10)*(tsv2-tsv)
    end if
    if (par_IMPSOLV(11).ne.0.0) then
      evec(13) = evec(13) + par_IMPSOLV(11)*sexv
    end if
...
!     ... "as is" for atoms
 ...
 &        (resname.eq.'I- ').OR.(resname.eq.'CS+').OR.&
 &        (resname.eq.'CLX').OR.(resname.eq.'NAX')) then
...
!
  if (fos_report.EQV..true.) then
    ...
    if (use_REMC.EQV..true.) then
      ...
      fn = 'N_'//nod(1:tl)//'_FOS_GROUPS.vmd'
    else if (use_MPIAVG.EQV..true.) then
      fn = 'FOS_GROUPS.vmd'
      if (myrank.ne.0) return
    end if
...
!
  else if ((resname.eq.'CLX')) then !.OR.(resname.eq.'CLX')) then
    at(rs)%nfosgrps = 1
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps))
    at(rs)%fosgrp(1)%val(1:3) = freesolv(rs,1:3)
    hh = 1
    allocate(at(rs)%fosgrp(1)%ats(hh))
    allocate(at(rs)%fosgrp(1)%wts(hh))
    allocate(at(rs)%fosgrp(1)%wts_limits(hh,2))
    !allocate(at(rs)%fosgrp(1)%val_limits(3,2))
    at(rs)%fosgrp(1)%nats = hh
    at(rs)%fosgrp(1)%ats(1) = at(rs)%bb(1)
    at(rs)%fosgrp(1)%wts(1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(1)%wts_limits(1,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(1)%wts_limits(1,2) = 0.0
    at(rs)%fosgrp(1)%val_limits(1:3,1) = FOS_CL(1:3) ! Martin : changed that to 
    at(rs)%fosgrp(1)%val_limits(1:3,2) = 0.0
!    at(rs)%fosgrp(1)%val_limits(1:3,1) = freesolv_limits(rs,1:3,1) ! Martin : changed that to 
!    at(rs)%fosgrp(1)%val_limits(1:3,2) = freesolv_limits(rs,1:3,2)
    which_fosg(at(rs)%bb(1)) = 1
    
    
  else if ((resname.eq.'NAX')) then !.OR.(resname.eq.'CLX')) then
    at(rs)%nfosgrps = 1
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps))
    at(rs)%fosgrp(1)%val(1:3) = freesolv(rs,1:3)
    hh = 1
    ...
    at(rs)%fosgrp(1)%wts_limits(1,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(1)%wts_limits(1,2) = 0.0
    at(rs)%fosgrp(1)%val_limits(1:3,1) = FOS_NA(1:3) ! Martin : changed that to 
    at(rs)%fosgrp(1)%val_limits(1:3,2) = 0.0
!    at(rs)%fosgrp(1)%val_limits(1:3,1) = freesolv_limits(rs,1:3,1) ! Martin : changed that to 
!    at(rs)%fosgrp(1)%val_limits(1:3,2) = freesolv_limits(rs,1:3,2)
    which_fosg(at(rs)%bb(1)) = 1
...
!
 ...
 &sidue ',resname,' (free) at the moment. Check back later(2).'
    call fexit()
  end if
!
end
...
!       add the NH-atoms
 ...
 &C-terminal residue ',resname,' at the moment. Check back later(3).'
      call fexit()
    end if
!
    if (sty.eq.'P') then
...
!
 ...
 &for the backbone solvation group for residue ',rs,' in helper_svgrps_middle(...). This is a bug.'
    ...
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1! Martin dont see the reason for this
    
    if ((seqflag(rs).eq.5).OR.(hni(rs).le.0)) then! if residue (is no proline ?) or if there is no hydrogen on the nitrogen atom for the backbone then
      hh = 3
    else
      hh = 4
    end if
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))! Martin allocates array of the size of nfosgrps to contain atomic information 
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))! Martin allocates array of the size of nfosgrps to contain weight information 
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh! Martin : The number of atoms in the last solvation group is made equal to hh.  &
    !Dont really understand why, it seem that the last value in the %fosgrp is where the backbone FOS parameters are going to be stored
    

!   first add CO on the previous residue
!   note we use a shift-by-100 trick to indicate that ci(rs-1) and oi(rs-1)
!   are not part of residue rs-1's fos-group list
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(1)= ci(rs-1)!martin : first atom of the last solvation group is set to ci(rs-1), which is the previous Co carbone atom
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(1)= 1.0/(1.0*hh)!martin : And then the weights are defined as one on the number of atoms in the solvation group (make sens)
    which_fosg(ci(rs-1)) = at(rs)%nfosgrps + 100! martin : Don't really get this, make no sens to me. Why would the reference number ofr the solvation group be the number of solvation groups that compose this residue ?????
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(2)= oi(rs-1)! martin : second atom ni the sovlation groups is the oxygen of the previous residue
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(2)= 1.0/(1.0*hh)! martin
    which_fosg(oi(rs-1)) = at(rs)%nfosgrps + 100! 
!   set the right ref. value
    if (seqflag(rs).eq.5) then
      if (resname2.eq.'FOR') then
...
!
  implicit none
!
  integer rs,imol,j,hh,tt,shf,shf2,shf3,shf5,shf7,lk,shfx1,shfx4,shfx2
  character(3) resname
  character sty,sty2
!
  shf = 0
...
!    
!! phosphoserine (phosphate dominates, strategy similar to 5' phosphate in nucleotides)
!! Martin : have not deleted the extra atom yet
!  else if (resname.eq.'S2P') then
!    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
!    hh = 4
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))FHEX
...
!    !which_fosg(at(rs)%sc(at(rs)%nsc)) = at(rs)%nfosgrps
 ...
 !    phosphoserine (phosphate dominates, strategy similar to 5' phosphate in nucleotides)
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_S1P(1:3)
    tt = 0
    do j=3-shf,at(rs)%nsc
      if (j.ge.(11-shf)) exit
      if ((j.eq.(8-shf)).OR.(j.eq.(9-shf))) cycle 
...
! Martin : have not deleted the extra atom yet
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_S2P(1:3)
    tt = 0
    do j=3-shf,at(rs)%nsc
      if (j.ge.(8-shf)) exit
      !if (j.eq.(5-shf)) cycle ! terminal O
...
!    which_fosg(at(rs)%sc(5-shf)) = at(rs)%nfosgrps
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_S2P(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_S2P(1:3) ! freesolv_limits(rs,1:3,1)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_S1P(1:3) ! FOS_ASH(1:3)
    ...
    do j=3-shf,at(rs)%nsc
      ...
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2)=1.0/(1.0*hh)
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
    end do
    ...
    do j=3-shf,at(rs)%nsc
      ...
      if ((j.eq.(8-shf)).OR.(j.eq.(9-shf))) cycle 
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)=1.0/(1.0*(hh-1))
    end do
    tt=tt+1
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)=0
...
! phosphothreonine (phosphate dominates, strategy combined of THR and 5' phosphate in nucleotides)
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_T1P(1:3)
    tt = 0
    do j=3-shf,at(rs)%nsc
      if (j.ge.(9-shf)) exit
      !if ((j.eq.(4-shf)).OR.(j.eq.(6-shf))) cycle ! CG, terminal O
      if ((j.eq.(4-shf))) cycle ! CG, terminal O
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
    end do
!    Martin : finishing the phosphates
    tt = tt + 1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(13)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
    which_fosg(at(rs)%sc(13)) = at(rs)%nfosgrps
...
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
!    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_S1P(1:3) - FOS_NUC_PO4D(1:3)
...
!   the CH3-part
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_CH3(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(1)= at(rs)%sc(4-shf)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(1)= 1.0/(1.0*hh)
    which_fosg(at(rs)%sc(4-shf)) = at(rs)%nfosgrps
    tt = 1
...
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
!    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = freesolv(rs,1:3) - FOS_S1P(1:3)
!    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(1)= at(rs)%sc(4-shf)
!    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(1)= 1.0/(1.0*hh)
!    which_fosg(at(rs)%sc(4-shf)) = at(rs)%nfosgrps
!    tt = 1
...
!
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_T2P(1:3)
    tt = 0
    do j=3-shf,at(rs)%nsc
      if (j.ge.(9-shf)) exit
      !if ((j.eq.(4-shf)).OR.(j.eq.(6-shf))) cycle ! CG, terminal O
...
!   the CH3-part
   ...
   at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_T2P(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_T1P(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_T2P(1:3)
    ! Proton off 
    ...
    which_fosg(at(rs)%sc(13)) = at(rs)%nfosgrps
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2)=1.0/(1.0*hh)
    !Easier to put the of weight here
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)=0.
...
!   the toluene part
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_TYR(1:3)
    tt = 0
    do j=3-shf,at(rs)%nsc
      !if ((j.eq.(9-shf)).OR.(j.eq.(16-shf3-shfx4))) cycle
      if (j.eq.(10-shf)) cycle
      if (j.eq.(11-shf)) cycle 
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
...
!   the toluene part
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_TYO(1:3) ! freesolv_limits(rs,1:3,1)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_TYR(1:3) !FOS_ASH(1:3)
    ...
    tt=tt+1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(16)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)= 0.0
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2)= 1.0/(1.0*hh)
    which_fosg(at(rs)%sc(16)) = at(rs)%nfosgrps
...
!   the toluene part
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_TYO(1:3)
    tt = 0
...
!   phosphate
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_Y1P(1:3)
    tt = 0
...
!   Martin : extra atom to be deleted
!   Phosphate
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    hh = 5
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_Y2P(1:3)
    tt = 0
    do j=9-shf,at(rs)%nsc
      if (j.ge.(14-shf)) exit
      !if (j.eq.(11-shf)) cycle ! terminal O
    ...
    ! Martin : Added that explicitly, cause no need to loop thorugh the next six elements if they are all exceptions
    !at(rs)%fosgrp(at(rs)%nfosgrps)%ats(6) = at(rs)%sc(20)
    !at(rs)%fosgrp(at(rs)%nfosgrps)%wts(6) = 1.0/(1.0*hh)
   ! which_fosg(at(rs)%sc(20)) = at(rs)%nfosgrps
...
!   phosphate
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_Y2P(1:3) ! FOS_NUC_PO4D(1:3)
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_Y2P(1:3) 
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_Y1P(1:3) 
    ...
    ! Actual proton
    tt=tt+1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(20)
    which_fosg(at(rs)%sc(20)) = at(rs)%nfosgrps
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2)=1.0/(1.0*hh)
    !Easier to put the of weight here
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)=0.
    ...
    do j=9-shf,at(rs)%nsc
      if (j.ge.(14-shf)) exit
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*(hh-1))
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1)=1.0/(1.0*(hh-1))
    end do
...
!   the polar N-C-NH
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_HIE(1:3) !freesolv(rs,1:3)
    tt = 0
    do j=2-shf,at(rs)%nsc
...
!   the polar NH-C-N
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_HID(1:3) !freesolv(rs,1:3)
    tt = 0
    do j=2-shf,at(rs)%nsc
      if (.NOT.((j.eq.(4-shf)).OR.(j.eq.(6-shf)).OR.&
...
!   the polar NH-C-NH
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_HIP(1:3)
    tt = 0
...
!   the polar NH-C-NH
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,3))
    
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = freesolv(rs,1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_HIP(1:3) !freesolv_limits(rs,1:3,1)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) =  FOS_HIE(1:3) 
    !at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,3) =  FOS_HID(1:3) !freesolv_limits(rs,1:3,2)
    tt = 0
 ...
 &              (j.eq.(7-shf)).OR.(j.eq.(10-shf3)))) cycle
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 1.0/(1.0*(hh-1))
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(13)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 0.0
    which_fosg(at(rs)%sc(13)) = at(rs)%nfosgrps
...
!   the polar NH-C-NH
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_HIP(1:3) !freesolv_limits(rs,1:3,1)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) =  FOS_HIE(1:3) 
    !at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,3) =  FOS_HID(1:3)
    tt = 0
    do j=2-shf,at(rs)%nsc
      if (.NOT.((j.eq.(4-shf)).OR.(j.eq.(6-shf)).OR.&
 &              (j.eq.(7-shf)).OR.(j.eq.(13-shf3-shfx2)))) cycle
    ...
    !Hydrogen 
    tt = tt + 1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(10)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 0.0
    which_fosg(at(rs)%sc(10)) = at(rs)%nfosgrps
    
  else if (resname.eq.'HIX') then
!   the polar NH-C-NH
      if (do_hsq.eqv..false.) then
          print *,"The use of HIX is reserved for HSQ simulations. Please change sequence file"
          call fexit()
      end if 
    ! Ok so this will be the HSQ compatible histidine.
    ! The idea here is to make the charge state of the neutral 
    !  states have 3 possible values : 1 for charged, 2 for HID, and 3 for HIE
    !  Then, assign those 3 limits array
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = freesolv(rs,1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_HIP(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) =  FOS_HIE(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,3) =  FOS_HID(1:3)
    tt = 0
 ...
 &              (j.eq.(7-shf)))) cycle
      ...
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 1.0/(1.0*(hh-1))
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,3) = 1.0/(1.0*(hh-1))
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
    end do
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 1.0/(1.0*(hh-1))
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,3) = 0.0
    which_fosg(at(rs)%sc(10)) = at(rs)%nfosgrps
...
!   the acid
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_GLH(1:3) - FOS_CH2(1:3)
    tt = 0
    do j=2-shf,at(rs)%nsc
      if (.NOT.((j.eq.(4-shf)).OR.(j.eq.(5-shf)).OR.(j.eq.(6-shf)).OR.&
 &                             (j.eq.(11-shf5)))) cycle
...
!   the Cbeta-methylene
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_CH2(1:3) 
    tt = 0
    do j=2-shf,at(rs)%nsc
      if (.NOT.((j.eq.(2-shf)).OR.(j.eq.7).OR.(j.eq.8))) cycle
      tt = tt + 1
...
! (partially protonated) glutamic acid: the difference between acetic acid and propionic acid is ~0.2kcal/mol,
!                             let's assign that small difference to the C-beta methylene
    else if (resname.eq.'GLX') then ! Martin : Note that in this version different UA model may not be supported
!   the acid
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    hh = 4 ! martin : changed that to 4
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
    !allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(3,2))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh ! There is  4 atoms in this solvation group
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_GLU(1:3) - FOS_CH2(1:3)
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) =  FOS_GLU(1:3) - FOS_CH2(1:3)
!    if (fos_corr.eqv..true.) then     
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_GLH(1:3) - FOS_CH2(1:3) 

    
!tyler pka: Hydrogen off
    hh = 3
    tt = 0
 ...
 &                             (j.eq.(11-shf5)))) cycle
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
     !at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j) ! Martin Moved those two line from the next loop to this one
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
...
!reset: Hydrogen on
    hh = 4
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh) = 0 ! Martin : two very unusefull lines
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,1)=0
    tt = 0
    do j=2-shf,at(rs)%nsc! Martin : removed -1
      if (.NOT.((j.eq.(4-shf)).OR.(j.eq.(5-shf)).OR.(j.eq.(6-shf)).OR.&
 &                             (j.eq.(11-shf5)))) cycle
      tt = tt + 1
      !at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2)=1.0/(1.0*hh)
!    if (fos_corr.eqv..true.) then 
    !   the Cbeta-methylene
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    hh = 3-shf2
    ...
    if (fos_corr.eqv..false.) then     
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = 0.0! freesolv_limits(rs,1:3,1)
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_CH2(1:3)
    else 
        at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_CH2(1:3)
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_CH2(1:3)! freesolv_limits(rs,1:3,1)
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_CH2(1:3)
    end if 
    tt = 0
    do j=2-shf,at(rs)%nsc
...
! Now the hydrogen, it gets its own fos group ! Martin : That is because the wts_limits apply to the entire solvation group.
!    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
!    hh = 4 !Martin : I want to put 4 here, so that the FOS for the entire side chain is indeed the correct one  (it was 1))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(1))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(1))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(3,2))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(1,2))
!    at(rs)%fosgrp(at(rs)%nfosgrps)%is_prot = .true.
!    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = 1 !Martin : Replaced hh by 1 
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_GLH(1:3)
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_GLU(1:3) ! Martin : put GLU here instead of zero, so I can 
!    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 0            ! Martin : Added line
!    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 1.0/(1.0*hh)
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_GLH(1:3)
!    j=at(rs)%nsc
!    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(1) = at(rs)%sc(j) ! Martin : That si just mapping back the solvation group atom to the same side chain atom 
!    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(1) = 1.0/(1.0*4) ! Martin : not sure this is needed, or at least should be 0
!    which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps !is this a problem? ! Martin : Don't see why it would be 
 !   the Cbeta-methylene !Martin : Not sure why this one would have its own solvation group
!    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
!    hh = 3-shf2
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
!    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
...
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = 0.0
!    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = freesolv_limits(rs,1:3,2) - FOS_ASH(1:3)
!    tt = 0
!    do j=2-shf,at(rs)%nsc
!      if (.NOT.((j.eq.(2-shf)).OR.(j.eq.7).OR.(j.eq.8))) cycle
!      tt = tt + 1
...
!   the carboxyl group
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_ASP(1:3) !freesolv(rs,1:3)
    tt = 0
    do j=2-shf,at(rs)%nsc
...
!
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_ASP(1:3) !freesolv_limits(rs,1:3,1) 
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_ASH(1:3) !freesolv_limits(rs,1:3,2)
    ! Martin : hydrogen on
    hh = 4
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    ...
    tt=tt+1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(8)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 0.0
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 1.0/(1.0*hh)
    which_fosg(at(rs)%sc(8)) = at(rs)%nfosgrps
...
!   the carboxyl group
    ...
    if (fos_corr.eqv..true.) then
        ...
        at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = 0.0
    end if 
    tt = 0
    do j=2-shf,at(rs)%nsc
      if (.NOT.((j.eq.(2-shf)).OR.(j.eq.7).OR.(j.eq.8))) cycle
...
!   the ammonium ion
    ...
    if (fos_corr.eqv..true.) then
        at(rs)%nfosgrps = at(rs)%nfosgrps + 1
        hh = 12
        if (ua_model.gt.0) hh = 4 
        allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%ats(hh))
        allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
        at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
        at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_NBU
        tt = 0

        do j=2-shf,at(rs)%nsc
            
          if ((j.eq.(6-shf)).OR.(j.ge.(at(rs)%nsc-2))) cycle
          tt = tt + 1
          at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
          at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
          which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
...
!
! deprotonated lysine: difference between full value and n-butane to amine group and n-butane to C4 part
  else if (resname.eq.'LYD') then
!   the amine group
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
    hh = 3
    ...
    allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts(hh))
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_LYD(1:3) - FOS_NBU
    tt = 0
    do j=2-shf,at(rs)%nsc
    ...
    do j=2-shf,at(rs)%nsc
      ...
      if ((j.eq.(6-shf)).OR.(j.ge.(at(rs)%nsc-1))) cycle
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
...
!   the amine group
    ...
    at(rs)%fosgrp(at(rs)%nfosgrps)%nats = hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%val(1:3) = FOS_LYS
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_LYS
    at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_LYD - FOS_NBU
    tt = 0
    do j=2-shf,at(rs)%nsc-1
      if (.NOT.((j.eq.(6-shf)).OR.(j.ge.(at(rs)%nsc-2)))) cycle
      tt = tt + 1
    ...
    tt = tt + 1
    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(17)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,1) = 1.0/(1.0*hh)
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(tt,2) = 0.0
    which_fosg(at(rs)%sc(17)) = at(rs)%nfosgrps
    ...
    if (fos_corr.eqv..true.) then
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = FOS_NBU
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_NBU
    else 
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,1) = 0.0
        at(rs)%fosgrp(at(rs)%nfosgrps)%val_limits(1:3,2) = FOS_NBU
    end if 

    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(1:3,1) = 1.0/hh
    at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(1:3,2) = 1.0/hh

    tt = 0
    do j=2-shf,at(rs)%nsc

      if ((j.eq.(6-shf)).OR.(j.ge.(at(rs)%nsc-2))) cycle !Martin : -1 to -2, may have to reverse
      tt = tt + 1
      at(rs)%fosgrp(at(rs)%nfosgrps)%ats(tt) = at(rs)%sc(j)
      at(rs)%fosgrp(at(rs)%nfosgrps)%wts(tt) = 1.0/(1.0*hh)
      which_fosg(at(rs)%sc(j)) = at(rs)%nfosgrps
...
! all other residues are not yet supported (or never will be)
 ...
 &n of residue ',resname,' at the moment. Check back later(4).'
    call fexit()
  end if
!   if ((do_pka_3.eqv..true.).and.(allocated(at(rs)%fosgrp).eqv..true.)) then
!     if (allocated(at(rs)%fosgrp(at(rs)%nfosgrps)%wts).eqv..true.) then
!       hh=size(at(rs)%fosgrp(at(rs)%nfosgrps)%wts)
!       if (allocated(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits).eqv..true.) then
!         allocate(at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(hh,2))
!         at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(:,1) = at(rs)%fosgrp(at(rs)%nfosgrps)%wts
!         at(rs)%fosgrp(at(rs)%nfosgrps)%wts_limits(:,2) = at(rs)%fosgrp(at(rs)%nfosgrps)%wts
!       end if
!     end if
!   end if
!
...
! all other residues are not yet supported (or never will be)
    ...
    write(ilog,*) 'Fatal. Implicit solvent does not support sidechai&
 &n of residue ',resname,' at the moment. Check back later(5).'
    call fexit()
  end if
!
end
