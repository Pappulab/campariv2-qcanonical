...
#include "mpif.h"
  ...
  integer i,j,dt(8),ierr,dummy(1),nktag,aone,kstag,atwo,athree
...
!
! lets get the current node hostname
...
!    write(*,*) 'M:Entered master.'
    ...
    write(ilog,*) 'Node hostname: ', hostname
    call write_license()
    write(ilog,*) 'Execution started at ',rc(2)(1:2),':',rc(2)(3:4),&
...
!   Sanity check against mismatch of nodes and replicas in MPI-RE
    if (use_REMC.EQV..true.) then
      if (mpi_nodes.ne.re_conditions) then
        write(ilog,*) 'Fatal: Number of nodes granted by mpirun (',&
 &mpi_nodes,') does not match number of replicas requested (',&
 &re_conditions,').'
        call fexit()
      end if
    end if
...
!      write(*,*) 'M:Sending to ',i
      call MPI_SEND(nkey,1,MPI_INTEGER,i,nktag,&
 &MPI_COMM_WORLD,ierr)
      call MPI_SEND(keysz,nkey,MPI_INTEGER,i,kstag,&
 &MPI_COMM_WORLD,ierr)
      do j=1,nkey
!        write(*,*) 'sending ',key(j)%line
        call MPI_SEND(key(j)%line,size(key(j)%line),MPI_CHARACTER,i,&
 &ktag+j,MPI_COMM_WORLD,ierr)
      end do
!      write(*,*) 'M:Waiting for completion from ',i
      call MPI_RECV(dummy,1,MPI_INTEGER,i,MPI_ANY_TAG,&
 &MPI_COMM_WORLD,mstatus,ierr)
      if (mstatus(MPI_TAG).ne.initag) then
        write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &' from slave ',i,'. Expected ',initag,'.'
        call fexit()
      end if
...
!    write(*,*) 'S:Slave ',myrank,': Waiting for master.'
    call MPI_RECV(dummy,1,MPI_INTEGER,masterrank,&
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.nktag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &' from master. Expected ',nktag,'.'
      call fexit()
    end if
    nkey = dummy(1)
    allocate(keysz(nkey))
    allocate(key(nkey))
    call MPI_RECV(keysz,nkey,MPI_INTEGER,masterrank,&
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.kstag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &' from master. Expected ',kstag,'.'
      call fexit()
    end if
    do j=1,nkey
      allocate(key(j)%line(keysz(j)))
      call MPI_RECV(key(j)%line,keysz(j),MPI_CHARACTER,masterrank,&
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
      if (mstatus(MPI_TAG).ne.(ktag+j)) then
        write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &' from master. Expected ',ktag+j,'.'
        call fexit()
      end if
    end do
    deallocate(keysz)
...
!
#include "mpif.h"
  integer i,istep,cycdims(2),atwo,ierr,masterrank
  integer mstatus(MPI_STATUS_SIZE),cyctag 
!
  atwo = 2
  masterrank = 0
  cyctag = 887
...
!
    ...
    do i=1,mpi_nodes-1
      call MPI_Send(cycdims,atwo,MPI_INTEGER,i,cyctag,MPI_COMM_WORLD,ierr)
    end do
!
  else
!
    call MPI_RECV(cycdims,atwo,MPI_INTEGER,masterrank,MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.cyctag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &' from master. Expected cycle limits (tag: ',cyctag,').'
      call fexit()
    end if
    curcyc_start = cycdims(1)
    curcyc_end = cycdims(2)
!
  end if
!
  end
...
!
subroutine MPI_REMaster(istep,ndump)
!
  use iounit
  use atoms
  use accept
...
!
#include "mpif.h"
  integer istep,ndump,azero,atwo,athree
  integer i,j,k,ierr,reptag,masterrank,msgtag(4)
  integer mstatus(MPI_STATUS_SIZE)
  RTYPE energy3,ee1,ee2,dumes(3),boxovol,force3
...
! every replica must recompute foreign energies as desired
  ...
  if (dynni.EQV..false.) then
    call lamenergy(mpi_evec,mpi_fvec)
  else
    call lamforce(mpi_evec,mpi_fvec)
  end if
  reol_all = reolallbu
  call CPU_Time(ee2)
  time_energy = time_energy + ee2 - ee1
!
! now let's combine energy vectors -> note that this will not scale well with large numbers of replicas
  call CPU_Time(ee1)
  if (myrank.eq.masterrank) then ! MARTIN : master rank is always set to 0, all other ranks should be zero
    mpi_emat(myrank+1,:) = mpi_evec(:)! MARTIN The energy matrix is set to the energy vector 
    do i=1,mpi_nodes-1
      ...
      if (re_nbmode.eq.2) then
        if ((i.lt.(re_conditions-1)).AND.(i.gt.0)) then! MARTIN here i is the number of replicas running (actually the number of nodes used in parralel)
...
!
  k = 2*mpi_nodes
  call MPI_ALLINTS1D(msgtag(1:4),azero,ierr,bcast,k,mpi_lmap(:,1),use_MPIcolls) ! always uses master as BC root
!
  j = mpi_lmap(mpi_nodes+myrank+1,1) - 1
  if (j.gt.myrank) then
    call MPI_SendCoordinates(j,afalse,dynni)
    call MPI_ReceiveCoordinates(j,afalse,dynni)
    call CPU_Time(ee2)
    time_comm = time_comm + ee2 - ee1
  else if (j.lt.myrank) then
    call MPI_ReceiveCoordinates(j,atrue,dynni)
    call MPI_SendCoordinates(j,atrue,dynni)
    call CPU_Time(ee2)
    time_comm = time_comm + ee2 - ee1
...
!
  call CPU_time(ee1)
  if (dynni.EQV..true.) call prt_curens(istep,boxovol,afalse)
  ...
  call CPU_time(ee2)
  time_analysis = time_analysis + ee2 - ee1
!
end
!
!----------------------------------------------------------------------------------------------
...
!
subroutine MPI_SendCoordinates(tnode,frombuf,dynni)
!
  use mpistuff
  use mcsums
  use system
...
!
#include "mpif.h"
  integer aone,atwo,tortag,rbctag,xyztag,cmdtag,cldtag,imdtag,ildtag,ierr,tnode,bufix
  RTYPE ee1,ee2
  logical frombuf,dynni
!
  call CPU_Time(ee1)
!
  tortag = 11
...
! send over structure under tags 11,12 or 13
  if ((fycxyz.eq.1).AND.(force_rexyz.EQV..false.)) then
    if (frombuf.EQV..false.) call transfer_torsn(aone,mpi_tvec(:,1))
    if (frombuf.EQV..false.) call transfer_rbc(aone,mpi_rbvec(:,1))
!    write(*,*) 'Slave ',myrank,': Sending torsions'
...
!
  if (dynni.EQV..true.) then
   ...
   & MPI_SendCoordinates(...). This is most certainly a bug.'
      ...
      if (fycxyz.eq.1) then
        if (frombuf.EQV..false.) call manage_ilds(mpi_dvec(:,:,1),aone)
        call MPI_SEND(mpi_dvec(:,:,bufix),(ndyntorsn+totrbd)*4,MPI_RTYPE,tnode,ildtag,MPI_COMM_WORLD,ierr)
      else if (fycxyz.eq.2) then
        if (frombuf.EQV..false.) call manage_clds(mpi_dvec(:,:,1),aone)
...
!
subroutine MPI_ReceiveCoordinates(tnode,buf,dynni)
!
  use mpistuff
  use mcsums
  use system
...
!
#include "mpif.h"
  integer aone,atwo,tortag,rbctag,xyztag,cmdtag,cldtag,imdtag,ildtag,ierr,tnode,i,dummy
  integer mstatus(MPI_STATUS_SIZE)
  RTYPE ee1,ee2,force3,energy3,otherT
  logical atrue,buf,dynni
!
  call CPU_Time(ee1)
!
  tortag = 11
...
!
  if ((fycxyz.eq.1).AND.(force_rexyz.EQV..false.)) then
    if (buf.EQV..true.) then
      call transfer_torsn(aone,mpi_tvec(:,2))
      call transfer_rbc(aone,mpi_rbvec(:,2))
    end if
...
!      write(*,*) 'Slave ',myrank,': Finalizing myself'
    call transfer_torsn(atwo,mpi_tvec(:,1))
    call transfer_rbc(atwo,mpi_rbvec(:,1))
    call fyczmat()
    do i=1,nmol
...
!
  if (dynni.EQV..true.) then
    if ((dyn_mode.eq.2).OR.(dyn_mode.eq.5)) then
      if (fycxyz.eq.1) then
        if (buf.EQV..true.) then
          call manage_imds(mpi_dvec(:,:,2),aone)
        end if
        ...
        call manage_imds(mpi_dvec(:,:,1),atwo)
      else if (fycxyz.eq.2) then
        if (buf.EQV..true.) then
          call manage_cmds(mpi_dvec(:,:,2),aone)
        end if
        ...
        call manage_cmds(mpi_dvec(:,:,1),atwo)
      else
        write(ilog,*) 'Fatal. Encountered unsupported choice of degrees of freedom (',fycxyz,') in&
   & MPI_ReceiveCoordinates(...). This is most certainly a bug.'
        call fexit()
      ...
      if (fycxyz.eq.1) then
        if (buf.EQV..true.) then
          call manage_ilds(mpi_dvec(:,:,2),aone)
        end if
        ...
        call manage_ilds(mpi_dvec(:,:,1),atwo)
      else if (fycxyz.eq.2) then
        if (buf.EQV..true.) then
          call manage_clds(mpi_dvec(:,:,2),aone)
        end if
        ...
        call manage_clds(mpi_dvec(:,:,1),atwo)
      else
        write(ilog,*) 'Fatal. Encountered unsupported choice of degrees of freedom (',fycxyz,') in&
   & MPI_ReceiveCoordinates(...). This is most certainly a bug.'
        call fexit()
  ...
  if (dynni.EQV..true.) then
    if (fycxyz.eq.1) then
      if (re_velmode.eq.1) then
        ...
        call rescale_velocities(otherT)
      end if
    else if (fycxyz.eq.2) then
      if (re_velmode.eq.1) then
        ...
        dummy = tnode + 1
        otherT = re_mat(dummy,Tdim)
        call rescale_cart_velocities(otherT)
      end if
    end if
  end if
  call CPU_Time(ee1)
...
!
#include "mpif.h"
  integer istep,azero,ixl,cpibu,medi(mpi_nodes,8)
  integer i,j,ierr,masterrank,k
  integer nmap(2*mpi_nodes),msgtag(4)
  RTYPE ee1,ee2,random
  logical bcast,dynni,afalse
...
!     deallocate old (this may be costly)
...
!
  k = 2*mpi_nodes
  call MPI_ALLINTS1D(msgtag(1:4),azero,ierr,bcast,k,nmap,use_MPIcolls) ! always uses master as BC root
  call CPU_Time(ee2)
  time_comm = time_comm + ee2 - ee1
!
...
!
! setup filehandle
  repbfile = 'N_000_PROBABILITIES.dat'
...
!
  ...
  call int2str(bk,bnrs2,azero) ! Martin : that is where the problem is 
...
!   we're just storing average energies
    re_olap(1,1) = re_olap(1,1) + esave
  else if (Tthere.EQV..true.) then
    if (dyn_mode.eq.1) then
      call lamenergy(evec,fvec)
    else
      call lamforce(evec,fvec)
    end if
    do i=i_start,i_end !1,mpi_nodes
      re_olap(i,1) = re_olap(i,1) + evec(i)*re_mat(i,Tdim)*gasconst - evec(myrank+1)/invtemp
      re_olap(i,2) = re_olap(i,2) + exp(-invtemp*(evec(i)*re_mat(i,Tdim)*gasconst - evec(myrank+1)/invtemp))
    end do
    ...
    do i=i_start,i_end !1,mpi_nodes
      re_olap(i,1) = re_olap(i,1) + (evec(i)-evec(myrank+1))/invtemp
      re_olap(i,2) = re_olap(i,2) + exp(-(evec(i)-evec(myrank+1)))
    end do
  else
    if (dyn_mode.eq.1) then
      call lamenergy(evec,fvec)
    ...
    do i=i_start,i_end !1,mpi_nodes
      re_olap(i,1) = re_olap(i,1) + (evec(i)-evec(myrank+1))/invtemp
      re_olap(i,2) = re_olap(i,2) + exp(-(evec(i)-evec(myrank+1)))
      re_olap(i,3) = re_olap(i,3) + fvec(i)
    end do
  end if
!
 62   format(1000000(g18.11,1x))
  ...
  if (prtout.EQV..true.) then
    if (Tthere.EQV..true.) then
      write(iremc,62)(re_mat(j,Tdim)*gasconst*evec(j),j=i_start,i_end)
    else
      write(iremc,62) (evec(j)/invtemp,j=i_start,i_end)
    end if
  end if
end
!
...
!
  integer OLNR
  parameter (OLNR=4)
  integer freeunit,ireol,i,j,tl,i_start,i_end,ii,jj
...
!
  ...
  reolfile = 'N_'//nod(1:tl)//'_OVERLAP.dat'
...
!
  integer i,masterrank,rmgtag,rmstag,contag1,contag2,segtag1,savtag3
  ...
  integer pctag6,pctag7,savtag2,contag3,segtag6,dssptag4,emtag1,emtag2,tbrktag,tbrktag2
  ...
  integer(8), ALLOCATABLE:: limap2(:,:),limap1(:)
  logical sayyes
!
  sayyes = .true.
  aone = 1
...
#include "mpif.h"
  ...
  RTYPE poly(nangrps,MPI_POLSZ),ee1,ee2
!
  masterrank = 0
  aone = 1
  polatag = 92
...
#include "mpif.h"
  ...
  RTYPE envals(MPI_ENSZ),ee1,ee2
!
  masterrank = 0
  entag = 72
!
  if (myrank.ne.masterrank) then
    write(*,*) 'Fatal. Called MPI_AVGWriteEn with slave node ('&
 &,myrank,').'
    call fexit()
  end if
!
 ...
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.entag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',mpi_cnt_en,'. Expected ',entag,'.'
      call fexit()
    end if
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
...
!
    call CPU_time(ee1)
    call MPI_SEND(envals,MPI_ENSZ,MPI_RTYPE,masterrank,&
 &entag,MPI_COMM_WORLD,ierr)
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
    time_analysis = time_analysis - ee2 + ee1
!
...
!
  if (myrank.ne.masterrank) then
    write(*,*) 'Fatal. Called MPI_AVGWriteAcc with slave node (',&
 &myrank,').'
    call fexit()
  end if
!
...
!
  call CPU_time(ee1)
  call MPI_SEND(accvals,MPI_ACCSZ,MPI_INTEGER,masterrank,&
 &acctag,MPI_COMM_WORLD,ierr)
  call CPU_time(ee2)
  time_comm = time_comm + ee2 - ee1
  time_analysis = time_analysis - ee2 + ee1
!
...
#include "mpif.h"
  ...
  integer ierr,mstatus(MPI_STATUS_SIZE)
  RTYPE envals(MPI_ENSSZ),ee1,ee2
!
  masterrank = 0
  enstag = 76
!
...
!
 ...
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.enstag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',mpi_cnt_ens,'. Expected ',enstag,'.'
      call fexit()
    end if
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
...
!
    call CPU_time(ee1)
    call MPI_SEND(envals,MPI_ENSSZ,MPI_RTYPE,masterrank,&
 &enstag,MPI_COMM_WORLD,ierr)
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
    time_analysis = time_analysis - ee2 + ee1
!
...
!
#include "mpif.h"
  integer masterrank,tortag,j,ttc,imol
  integer ierr,mstatus(MPI_STATUS_SIZE)
  RTYPE ee1,ee2,offset
  logical ismember
!
  tortag = 71
  masterrank = 0
!
  if (myrank.ne.masterrank) then
    write(*,*) 'Fatal. Called MPI_AVGWriteTor with slave node (',&
 &myrank,').'
    call fexit()
  end if
!
!
 10   format(i12,1x,20000(f9.3,1x))
!
...
!
#include "mpif.h"
  integer masterrank,tortag,ttc,imol
  integer ierr
  RTYPE ee1,ee2,offset
  logical ismember
!
...
#include "mpif.h"
  ...
  integer ierr,mstatus(MPI_STATUS_SIZE)
  RTYPE polvals(MPI_POLSZ),ee1,ee2
...
!
    call CPU_time(ee1)
    call MPI_RECV(polvals,MPI_POLSZ,MPI_RTYPE,mpi_cnt_pol,&
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.poltag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',mpi_cnt_pol,'. Expected ',poltag,'.'
      call fexit()
    end if
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
    time_analysis = time_analysis - ee2 + ee1
    write(ipolmr,10) nstep,polvals(1),polvals(2),polvals(3),&
 &polvals(4),polvals(5),polvals(6),polvals(7),polvals(8)
!
  end if
!
end
...
!
    call CPU_time(ee1)
    call MPI_SEND(polvals,MPI_POLSZ,MPI_RTYPE,masterrank,&
 &poltag,MPI_COMM_WORLD,ierr)
    call CPU_time(ee2) 
    time_comm = time_comm + ee2 - ee1
    time_analysis = time_analysis - ee2 + ee1
!
...
!
  if (myrank.ne.masterrank) then
    write(*,*) 'Fatal. Called MPI_AVGWriteSAV with slave node (',&
 &myrank,').'
    call fexit()
  end if
!
...
!
 24   format(i12,1x,5i11)
  do i=1,mpi_nodes-1
    call MPI_RECV(savtmp,2+savreq%nats,MPI_RTYPE,i,&
 &MPI_ANY_TAG,MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.savtag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',i,'. Expected ',savtag,'.'
...
!
  ...
  time_analysis = time_analysis - ee2 + ee1
!
 47   format(i10,1000000(1x,g15.7))
...
#include "mpif.h"
  ...
  integer ierr
  RTYPE savtmp(2+savreq%nats),ee1,ee2,tsv,dummy
  logical sayno
!
  sayno = .false.
  masterrank = 0
...
!
  call CPU_time(ee1)
  call MPI_SEND(savtmp,2+savreq%nats,MPI_RTYPE,masterrank,&
 &savtag,MPI_COMM_WORLD,ierr)
  call CPU_time(ee2)
  time_comm = time_comm + ee2 - ee1
  time_analysis = time_analysis - ee2 + ee1
!
...
!
  if (mstatus(MPI_TAG).ne.strtag) then
    write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',mpi_cnt_xyz,'. Expected ',strtag,'.'
    call fexit()
  end if
!
end
...
#include "mpif.h"
  ...
  integer ierr,mstatus(MPI_STATUS_SIZE)
  RTYPE ttt(ntorsn),ee1,ee2
!
  trcvtag = 70
  masterrank = 0
!
  if (myrank.ne.masterrank) then
    write(*,*) 'Fatal. Called MPI_AVGWriteTRCV with slave node (',&
 &myrank,').'
    call fexit()
  end if
!
...
!   use the values passed through argument
    ...
    call CPU_time(ee1)
    call MPI_RECV(ttt,dc,MPI_RTYPE,mpi_cnt_trcv,MPI_ANY_TAG,&
 &MPI_COMM_WORLD,mstatus,ierr)
    if (mstatus(MPI_TAG).ne.trcvtag) then
      write(*,*) 'Fatal. Received bad message (',mstatus(MPI_TAG),&
 &') from slave ',mpi_cnt_trcv,'. Expected ',trcvtag,'.'
      call fexit()
    end if
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
...
!
    call CPU_time(ee1)
    call MPI_SEND(ttt,dc,MPI_RTYPE,masterrank,&
 &trcvtag,MPI_COMM_WORLD,ierr)
    call CPU_time(ee2)
    time_comm = time_comm + ee2 - ee1
    time_analysis = time_analysis - ee2 + ee1
!
