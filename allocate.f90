...
!
 ...
 &      (allocated(molfrzidx).EQV..false.)) then
...
!#tyler pka:
    allocate(molvol_limits(nmoltyp,2,3))
...
! some flags
!
    allocate(tmp_nbl(imax))
    do i=1,imax
      allocate(tmp_nbl(i)%nbl(nseq))
      allocate(tmp_nbl(i)%idx(nseq))
      allocate(tmp_nbl(i)%k1(nseq,3))
      allocate(tmp_nbl(i)%k2(nseq,3))
      allocate(tmp_nbl(i)%svl(nseq,3))
      allocate(tmp_nbl(i)%svec(3,nseq))
      allocate(tmp_nbl(i)%dvec(nseq,3))
      allocate(tmp_nbl(i)%d1(nseq))
    end do
!
    do i=1,nseq
      seqpolty(i) = 'X'
...
!
 ...
 &      (allocated(glob_rslst).EQV..false.).OR.&
 &      (allocated(rs_nbl).EQV..false.).OR.&
 &      (allocated(rsp_vec).EQV..false.)) then
 ...
 &fix or report this problem.'
      ...
      if (allocated(rs_nbl).EQV..true.) then
        do i=1,nseq
          ...
          if (allocated(rs_nbl(i)%nblr).EQV..true.) then
            ...
            deallocate(rs_nbl(i)%lrsvec)
          end if
          if (allocated(rs_nbl(i)%gnblr).EQV..true.) then
            deallocate(rs_nbl(i)%gnblr)
            deallocate(rs_nbl(i)%glrsvec)
          end if
    ...
    if (allocated(tmp_nbl).EQV..false.) then
      write(ilog,*) 'Fatal. This is a bug. Some or all temporary neighbor&
 & list arrays were not properly allocated, or prematurely freed. Please &
 &fix or report this problem.'
      call fexit()
    end if
    do i=1,imax
      if ((allocated(tmp_nbl(i)%nbl).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%idx).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%k1).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%k2).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%svl).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%svec).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%dvec).EQV..false.).OR.&
 &        (allocated(tmp_nbl(i)%d1).EQV..false.)) then
        write(ilog,*) 'Fatal. This is a bug. Some or all temporary neighbor&
 & list arrays were not properly allocated, or prematurely freed. Please &
 &fix or report this problem.'
        call fexit()
      else
        deallocate(tmp_nbl(i)%nbl)
        deallocate(tmp_nbl(i)%idx)
        deallocate(tmp_nbl(i)%k1)
        deallocate(tmp_nbl(i)%k2)
        deallocate(tmp_nbl(i)%svl)
        deallocate(tmp_nbl(i)%svec)
        deallocate(tmp_nbl(i)%dvec)
        deallocate(tmp_nbl(i)%d1)
      end if
    end do
    deallocate(tmp_nbl)
    if (use_IMPSOLV.EQV..true.) then
...
! a little buffer-space, too)
  ...
  if (mode.eq.1) then
    ...
    allocate(mass(natts))
! #tyler pka
!     allocate(pka_limits_scaled(natts))
    if ((do_pka_3.eqv..true.).or.(do_hsq.eqv..true.)) then 
     allocate(blen_limits(natts,3))
     allocate(bang_limits(natts,3))
     allocate(temp_save(natts))
     allocate(temp_bio_save(natts))
  end if 
...
!
  ...
  use molecule
  use system
  use params ! Martin : out of lazyness
  use movesets
!
  implicit none
  character*1000 WW
...
! #tyler pka
     if ((do_pka_2.eqv..true.).or.(do_hsq.eqv..true.)) then
      allocate(atstv_limits(n+20,3))
      allocate(atsavred_limits(n+20,3))! Martin : put the conditional back )
      allocate(atsavprm_limits(n,6,3))
      allocate(atr_limits(n+20,3))
      allocate(atvol_limits(n+20,3))
      allocate(atsavmaxfr_limits(n+20,3))
      allocate(atbvol_limits(n+20,3))
      
      printed_limits=.false.
     end if
    allocate(atsavmaxfr(n+20))
    
!
    if ((use_IMPSOLV.EQV..true.).OR.(savcalc.le.nsim)) then
      allocate(atsavprm(n,6))
...
!#tyler pka
      if ((do_pka.eqv..true.).or.(do_hsq.eqv..true.)) then 
          allocate(atq_limits(n,4))
          atq_limits(:,:)=0.0 ! AMrtin : Added 
      end if 
...
!
 ...
 & fix or report this problem.'
    ...
    if (use_IMPSOLV.EQV..true.) then
      if ((allocated(svte).EQV..false.).OR.&
 &        (allocated(svbu).EQV..false.).OR.&
...
!
  if (mode.eq.1) then
!    allocate(grid%nblist(grid%dim(1)*grid%dim(2)*grid%dim(3)+1,MAXGRPPERGP))
!    allocate(grid%gpnblist(grid%dim(1)*grid%dim(2)*grid%dim(3)+1,MAXNGPPERGP))
 ...
 &dimensions. Disabling spatial density analysis.'
    ...
    if (bnd_shape.eq.1) then
      ...
      if (bnd_type.eq.1) then
        ...
        write(ilog,*) 'Fatal. Spatial density restraints are not yet compatible with nonperiodic, &
 &rectangular boxes. Please check back later.'
        call fexit()
      end if
    else if (bnd_shape.eq.2) then
      emgrid%dim(:) = ceiling(3*bnd_params(4)/emgrid%deltas(:))
      emgrid%origin(:) = bnd_params(1:3) - 1.5*bnd_params(4)
    else if (bnd_shape.eq.3) then
      write(ilog,*) 'Fatal. Cylindrical systems are not yet supported for spatial density restraints. This is an omission bug.'
      call fexit()
    end if
...
!
    allocate(emgrid%vol(emgrid%dim(1),emgrid%dim(2),emgrid%dim(3)))
...
!
    ...
    if ((allocated(emgrid%vol).EQV..false.).OR.&
 &      (allocated(emgrid%mass).EQV..false.).OR.&
 ...
 &      (allocated(embsplbu).EQV..false.).OR.&
...
!creates a table that is AxB, where A is the number of molecules and 
!B is the length of the longest molecule  #tyler
!       if (use_ZSEC_MinL(1).EQV..true.) then
        allocate(z_alpha_vector(nmol,maxval(rsmol(:,2)-rsmol(:,1))+1))
!       end if
!       if (use_ZSEC_MinL(2).EQV..true.) then
        allocate(z_beta_vector(nmol,maxval(rsmol(:,2)-rsmol(:,1))+1))
!       end if
...
!#tyler pka
    allocate(lj_eps_limits(n_ljtyp,n_ljtyp,3))
    allocate(lj_sig_limits(n_ljtyp,n_ljtyp,3))
    allocate(lj_eps_14_limits(n_ljtyp,n_ljtyp,3))
    allocate(lj_sig_14_limits(n_ljtyp,n_ljtyp,3))
    allocate(lj_rad_limits(n_ljtyp,3))
...
!
 ...
 &eport this problem.'
      ...
      !deallocate(lj_sig_14) ! Martin : Deallocation stalled, so I commented it, to be addressed later
...
!
  if (mode.eq.1) then
    ...
    allocate(cart_f_lr(n,3))
    allocate(cart_v(n,3))
    cart_v(:,:) = 0.0
#ifdef ENABLE_THREADS

 
    allocate(tmpcf(n,3,3,thrdat%maxn)) ! atoms,dimensions (x,y,z),range (near,tr,lr),thread #
#else
    allocate(tmpcf(n,3,3,2))
#endif
    allocate(cart_a(n,3))
    allocate(cart_frz(n,3))
    cart_frz(:,:) = .false.
    allocate(sav_dr(n,3))
    allocate(scrq_dr(n,3))
    allocate(sisa_dr(MAXNB,3,n))
    allocate(sisq_dr(MAXNB,3,n))
    allocate(sisa_nr(n))
    sisa_nr(1:n) = 0
    allocate(sum_scrcbs(n))
    allocate(sum_scrcbs_tr(n))
    allocate(sum_scrcbs_lr(n))
    allocate(sisa_i(MAXNB,n))
...
!
 ...
 &      (allocated(cart_f_lr).EQV..false.).OR.&
 ...
 &      (allocated(sisa_dr).EQV..false.).OR.&
 &      (allocated(sisq_dr).EQV..false.).OR.&
 &      (allocated(sisa_nr).EQV..false.).OR.&
 &      (allocated(tmpcf).EQV..false.).OR.&
 &      (allocated(sum_scrcbs).EQV..false.).OR.&
 &      (allocated(sum_scrcbs_tr).EQV..false.).OR.&
 &      (allocated(sum_scrcbs_lr).EQV..false.).OR.&
 &      (allocated(sisa_i).EQV..false.).OR.&
...
!  
  if (mode.eq.1) then
    ...
    allocate(numberhistogram(nmoltyp, 0:nmol))
    numberhistogram(:,:) = 0
  else if (mode.eq.2) then
...
!
    ...
    if (allocated(bsplbu).EQV..true.) deallocate(bsplbu)
